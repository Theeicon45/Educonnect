  ...
  import { FaArrowLeft, FaEllipsisVertical, FaPaperPlane, FaPlus } from "react-icons/fa6";
...
useEffect(() => {
  ...
  const fetchMessages = async () => {
    const now = Date.now();
    if (now - lastFetchTime < 5000) return; // Ensures only one call per 5 seconds
  ...
  const interval = setInterval(fetchMessages, 1000); // Runs every second but fetches only every 5s

  return () => clearInterval(interval);
}, [selectedChat, lastFetchTime]); 
            ...
            <div className="flex-1 p-4 overflow-y-auto bg-gray-100">
              {selectedChat.messages?.map((msg, index) => (
                <div key={index} className={`flex ${msg.sender_id === userId ? "justify-end" : "justify-start"} mb-2`}>
                  <button onClick={() => toggleOptionsMenu(index)} className="text-white focus:outline-none">
                        <FaEllipsisVertical />
                      </button>
          ...
          <div className="divide-y">
            {chats.map((chat) => (
              <div key={chat.id} onClick={() => handleChatSelect(chat)} className="flex items-center p-3 cursor-pointer hover:bg-gray-100">
                ...
                <div className="text-xs text-gray-400">{new Date(chat.last_updated).toLocaleTimeString([],{ hour: '2-digit', minute: '2-digit' })}</div>
              </div>
            ))}
          </div>
        )}
